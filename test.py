...
class VehicleDroneDeliveryOptimizer:
    ...
    # ========= 可视化 ========= #
    def visualize_convergence(self, history):
        """可视化遗传算法收敛趋势"""
        plt.figure(figsize=(12, 8))
        plt.plot(range(len(history)), history, 'b-', linewidth=2)
        plt.title('遗传算法收敛趋势 - 每代最优解', fontsize=16)
        plt.xlabel('迭代次数', fontsize=14)
        plt.ylabel('最优成本 (元)', fontsize=14)
        plt.grid(True, alpha=0.3)
        
        # 添加最小成本标记
        min_cost = min(history)
        min_gen = history.index(min_cost)
        plt.scatter(min_gen, min_cost, color='red', s=100, zorder=5)
        plt.annotate(f'最小成本: {min_cost:.2f} 元',
                     xy=(min_gen, min_cost),
                     xytext=(min_gen+5, min_cost+50),
                     arrowprops=dict(facecolor='black', shrink=0.05, width=1, headwidth=5))
        
        plt.tight_layout()
        plt.savefig('遗传算法收敛趋势.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    # 可视化遗传算法收敛趋势
    opt.visualize_convergence(hist)
